function whatToDo(option){   									//Primero preguntamos qué se quiere hacer a través de un prompt.
	do{
		...
		if(answer != ""){
			if (answer == 1 || answer == 2){
				...
				alert("Ingrese una opción válida");				// Un alert advertirá que ingrese una opción válida.
			}
		}
	} while (answer == "" || (answer != "1" && answer != "2"));	// Condición que hará que nuestra función se detenga.
}
...
function getPhrase(){				// Función para solicitar la frase, y verificar que no posea números ni esté vacía.
	...
	}while (phrase == "" || containsNum(phrase) == true || isNaN(phrase) == false);    	// Condición que hará que nuestra 
																						// función se detenga		
